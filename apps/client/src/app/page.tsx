...
import { getBoardsForUser } from "@synapse/firebase-admin";
...
function SignIn({ provider, ...props }: { provider?: string; [key: string]: any }) {
  ...
  return (
    <form action={async () => { "use server"; await signIn(provider); }}>
      ...
      <div className="w-full max-w-2xl p-8 space-y-6 bg-white rounded-lg shadow-md">
        {user ? (
          <div>
            <div className="flex justify-between items-center">
              <div className="text-left">
                <h1 className="text-2xl font-bold tracking-tight text-gray-900"> Welcome, {user.name || "User"}! </h1>
            ...
            <div className="mt-8 border-t pt-6">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">Create or Join a Board</h2>
              <CreateBoardForm />
            </div>
          ...
          <div className="text-center">
            <h1 className="text-3xl font-bold tracking-tight text-gray-900"> Synapse Whiteboard </h1>
            <p className="mt-2 text-sm text-gray-600"> Your real-time collaborative canvas. </p>
            <div className="space-y-4 mt-8">
              <p className="text-center text-gray-600">Please sign in to continue</p>
              <SignIn provider="google" className="w-full px-4 py-2 text-sm font-medium text-white bg-teal-600 rounded-md hover:bg-teal-700" />
