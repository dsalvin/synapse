...
import type { Shape, PresenceUser, PenShape, RectShape, CircleShape, NoteShape as NoteShapeType, DiamondShape as DiamondShapeType, ConnectorShape } from '@/types/board.d';
...
const LiveCursor = ({ x, y, name }: { x: number; y: number; name: string }) => (
    <Text text={`\uD83D\uDC49 ${name}`} x={x + 10} y={y + 10} fill="blue" fontSize={16} />
);

const WhiteboardCanvas = ({ boardId, user }: { boardId: string, user: User & { id: string } }) => {
  const { history, historyIndex, setShapes, updateShapeInHistory, activeTool, fillColor } = useAppStore();
  ...
  const { cursors, sendCursorPosition, sendObject } = useWhiteboard(boardId, user, handleBoardLoad, handleAddShapes, handleUpdateShape, handleDeleteShapes, handlePresenceUpdate);
  ...
  const getPointerPos = () => {
      const stage = stageRef.current; if (!stage) return null; const pointerPos = stage.getPointerPosition(); if (!pointerPos) return null;
      return { x: (pointerPos.x - stage.x()) / stage.scaleX(), y: (pointerPos.y - stage.y()) / stage.scaleY(), };
  };

  const zoomStage = (direction: 'in' | 'out') => {
    const stage = stageRef.current; if (!stage) return; const scaleBy = 1.1; const oldScale = stage.scaleX();
  ...
  const handleStageMouseDown = (e: Konva.KonvaEventObject<MouseEvent>) => {
    ...
    if (activeTool === 'eraser' && didClickOnShape) {
      const idToDelete = e.target.id();
      if (idToDelete) { handleDeleteShapes([idToDelete]); sendObject({ type: 'OBJECT_DELETE', payload: [idToDelete] }); } return;
    }
    ...
    if (activeTool === 'pen') {
        ...
        const newShape: PenShape = { id, type: 'pen', points: [pos.x, pos.y], stroke: fillColor, x: 0, y: 0 };
  ...
  const handleStageMouseMove = (e: Konva.KonvaEventObject<MouseEvent>) => {
    ...
    if (isPanning.current) {
        const stage = e.target.getStage(); if (stage && pos) {
    ...
    if (activeTool === 'connector' && isConnecting.current && pos) {
  ...
  const handleStageMouseUp = (e: Konva.KonvaEventObject<MouseEvent>) => {
    ...
    if (activeTool === 'pen' && isDrawing.current) {
      ...
      if (line) { sendObject({ type: 'OBJECT_UPDATE', payload: line }); }
    }
  ...
  const handleTextEditEnd = () => {
    if (textAreaRef.current) {
        ...
        setShapes(shapes.map(s => {
            if (s.id === textEdit.id && s.type === 'note') { return { ...s, text: payload.text }; }
            return s;
        }));
...
export default function Whiteboard({ boardId }: { boardId: string }) {
  ...
  if (status === "loading") { return ( <div className="w-full h-screen flex items-center justify-center"> <p className="text-2xl font-semibold">Loading Session...</p> </div> ); }
  if (status === "unauthenticated" || !session?.user) { return ( <div className="w-full h-screen flex items-center justify-center"> <p className="text-2xl font-semibold">Access Denied. Please sign in.</p> </div> ); }
